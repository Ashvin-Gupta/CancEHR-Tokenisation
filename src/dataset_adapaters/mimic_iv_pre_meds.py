...
def main(input_dir: Path, output_dir: Path, do_overwrite: bool | None = None, do_copy: bool | None = None):
    ...
    # ================================================================================
    # SECTION 1: Check if processing is already complete
    # ================================================================================
    ...
    # ================================================================================
    # SECTION 2: Discover all input files recursively
    # ================================================================================
    # Find all files in subdirectories (hosp/*.csv, icu/*.csv, etc.)
    all_fps = list(input_dir.rglob("*/*.*"))
    # Also find any files directly in the root directory
    all_fps += list(input_dir.rglob("*.*"))

    # ================================================================================
    # SECTION 3: Initialize tracking dictionaries
    # ================================================================================
    # Track files that need data from other files for processing
    dfs_to_load = {}
    # Avoid loading the same file multiple times
    seen_fps = {}

    # ================================================================================
    # SECTION 4: First pass - Process files that don't need dependencies
    # ================================================================================
    for in_fp in all_fps:
        # Get the standardized prefix (e.g., "hosp/admissions" from "/path/hosp/admissions.csv.gz")
        pfx = get_shard_prefix(input_dir, in_fp)

        # Find the actual file and appropriate read function for this prefix
        try:
        ...
        # For CSV files, use schema inference to handle large files properly
        ...
        # ========================================
        # CASE 1: Files that need no processing - just copy/symlink
        # ========================================
        ...
        # ========================================
        # CASE 2: Files that need processing defined in FUNCTIONS
        # ========================================
            ...
            # If no dependencies needed, process immediately
            if not need_df:
                st = datetime.now()
    ...
    for df_to_load_pfx, fps_and_cols in dfs_to_load.items():
        ...
        # Optimize loading by only reading needed columns for compressed files
        ...
        # Process each file that depends on this reference data
        for fp in fps:
            ...
            # Apply the transformation function with the reference data
            processed_df = fn(fp_df, df)
            write_lazyframe(processed_df, out_fp)
            logger.info(f"    Processed and wrote to {str(out_fp.resolve())} in {datetime.now() - fp_st}")

    # ================================================================================
    # SECTION 6: Third pass - Special processing for ICD code files
    # ================================================================================
    for pfx, fn in ICD_DFS_TO_FIX:
        ...
        # Don't infer schema for non-parquet files to preserve string types
        if fp.suffix != ".parquet":
            read_fn = partial(read_fn, infer_schema=False)

        st = datetime.now()
        logger.info(f"Processing {pfx}...")
        # Process ICD codes to add proper dot notation
        processed_df = (
            read_fn(fp)
            .collect()
            .with_columns(
...
if __name__ == "__main__":
    ...
    mimic_iv_dir = Path(os.getenv("MIMIC_IV_DIR"))
    output_dir = Path("/home/joshua/data/mimic_meds")

    main(input_dir=mimic_iv_dir, output_dir=output_dir)