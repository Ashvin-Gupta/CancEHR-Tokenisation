...
class WordLevelTokenizer(Tokenizer):
    ...
    def train(self, event_files: List[str], preprocessors: List[BasePreprocessor], postprocessors: List[Postprocessor]) -> None:
        ...
        for file_path in tqdm(event_files, total=len(event_files)):
            ...
            # Apply postprocessors to the events
    ...
    def encode(self, event_filepath: str, preprocessors: List[BasePreprocessor], postprocessors: List[Postprocessor]) -> List[dict]:
        ...
        timelines = []
        for subject_id, subject_strings, subject_timestamps in zip(subject_ids, subject_event_lists["strings"], subject_event_lists["timestamps"]):
                
            timelines.append(
                {
                    "subject_id": subject_id,
                    "tokens": [self.vocab_map[token] if token in self.vocab_map else self.vocab_map[self.unknown_token] for token in subject_strings],
